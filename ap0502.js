...
function init() {
  ...
  function setUvs(f, x, s) {
    ...
    break;
    case 4: // 4段目
      uvs.array[f]   = x0 + dx * x;          // 左上
      uvs.array[f+1] = y0 - 2 * dy;          // 上
      uvs.array[f+2] = x0 + dx * (x - 2);    // 右上
      uvs.array[f+3] = y0 - 2 * dy;          // 中
      uvs.array[f+4] = x0 + dx * (x - 1);    // 右下
      uvs.array[f+5] = y0 - 3 * dy;          // 下
    break;
    }
  }
  // 関数の適用
  ...
  function render() {
    ...
    // テクスチャの切り替え
    if (controls.test) {
      material.map = testTexture; // testがtrueのときはmoonTesterを表示
    } else {
      material.map = moonTexture; // testがfalseのときはmoonを表示
    }
    material.map.needsUpdate = true; // テクスチャを更新
    // 物体の回転
    if (controls.rotate) {
